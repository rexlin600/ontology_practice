# Day 22｜Agent 与 Action：自动化要有护栏、限额与可中止点
...
库存 Agent 可以每天检查未来手术排程、识别低库存耗材，并生成补货草案；它可以提交给库管员审批，但不应自行下采购单或替代临床判断。
...
仅读取本院区未来 72 小时排程与已授权库存；单 SKU 建议不超过安全库存上限；涉及新供应商、超过金额阈值或数据质量异常时只生成草案并要求审批。
...
把 Agent 成功率等同于自动执行率。高风险场景中，正确升级给人也是成功。
