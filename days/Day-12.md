# Day 12｜Workshop 思维：先让一线人员点选，再谈自动化

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 理解低代码工作台的适用边界：协作、试用和流程承载，而非取代核心系统。
...
工作台适合：对象检索、待办、例外处理、批量审阅、只读汇总。高风险写回应逐步引入审批和集成测试。

## 把它放进真实现场

在两周原型中，可用对象视图让护士手工确认“已通知导管室”，而不是一开始就双向写回 HIS。这样能先检验流程是否有价值，也降低接入风险。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  ObjectView["对象视图（Object View）"] --> UserReview["UserReview"]
  UserReview["UserReview"] --> DraftAction["DraftAction"]
  DraftAction["DraftAction"] --> ApprovedWriteback["ApprovedWriteback"]
...
设计一个“待确认的高危患者”队列：筛选、排序、操作各是什么？

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

筛选：high_risk=true 且确认状态非 Completed；排序：剩余时限升序；操作：查看证据、确认通知、升级负责人、标记误报。每次操作记录人和时间。
...
把原型里的手工步骤视为失败。早期手工确认恰恰能暴露规则是否可靠。
