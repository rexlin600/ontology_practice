# Day 24｜行业案例二：医疗器械与耗材，把“批号”连接到患者安全

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 理解供应与临床使用的交叉建模，追溯风险如何落到具体对象。
...
批号追溯要求记录事件：何时入库、何时预留、何时使用、何时报废；仅在库存表上覆盖数量无法追溯。

## 把它放进真实现场

一批导管被召回，库房系统只知道还有多少；临床团队要知道哪些病例使用过、是否有替代库存、哪些预约需要调整。Ontology 可以连接 InventoryLot、Procedure、Patient（受权限控制）和SupplierNotice。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  SupplierNotice["SupplierNotice"] --> InventoryLot["库存批次（Inventory Lot）"]
  InventoryLot["库存批次（Inventory Lot）"] --> Procedure["Procedure"]
  Procedure["Procedure"] --> FollowUpTask["FollowUpTask"]
...
为召回事件设计一个 Action 和一个审计问题。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

Action：创建受影响批次的冻结与病例随访待办；审计问题：“该批次涉及哪些已完成手术、谁在何时完成了通知与复核？”
...
把库存数量与批次混为一谈。数量回答“还剩多少”，批次回答“这些具体物品去了哪里”。
