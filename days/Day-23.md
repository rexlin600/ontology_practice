# Day 23｜行业案例一：把诊疗路径建成可协作的对象网络

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 将前 22 课迁移到医院路径管理，理解何处必须保留临床边界。
...
临床价值不来自“更多数据”，而来自更早发现例外、更少重复确认和更清楚的责任交接。

## 把它放进真实现场

以胸痛路径为例：Patient 与 Encounter 是中心；Observation 提供证据；RiskAssessment 提供可解释建议；CarePathway 记录协作状态；Action 只处理通知、待办与记录，不取代诊断和医嘱。
...
flowchart LR
  ...
  Encounter["一次就诊（Encounter）"] --> RiskAssessment["风险评估（Risk Assessment）"]
  RiskAssessment["风险评估（Risk Assessment）"] --> CarePathway["诊疗路径（Care Pathway）"]
  CarePathway["诊疗路径（Care Pathway）"] --> TeamAction["TeamAction"]
...
为你所在科室任选一条路径，写出一个“超时风险”对象。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

例如术前抗凝评估：对象为 PreOpAssessment，字段含手术时间、最后一次用药、肾功能、评估状态；当距手术不足 24 小时且评估未完成，生成需人工确认的例外。
...
把教学案例当临床决策支持的替代品。任何上线规则都要经过本院临床、信息、合规与安全流程验证。
