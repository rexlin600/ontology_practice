# Day 05｜链接是业务语言：从外键到“谁与谁在什么条件下相关”

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 能用链接表达诊疗、资源和责任关系，并写清基数与有效期。
...
每条链接应说明方向、基数、何时生效、何时失效，以及是否允许人工修正。

## 把它放进真实现场

患者与就诊是一对多；一次就诊可产生多项检查；一个导管室时段可关联一台手术。链接不只是 ID 拼接，它决定用户能否从患者看到责任医生、待办事项和资源瓶颈。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Patient["患者（Patient）"] --> EncounterEncounter["Encounter; Encounter"]
  EncounterEncounter["Encounter; Encounter"] --> ProcedureProcedure["Procedure; Procedure"]
  ProcedureProcedure["Procedure; Procedure"] --> CathLabSlot["CathLabSlot"]
...
画出 Patient、Encounter、Clinician、Procedure、InventoryLot 五个对象的关系。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

Patient—has→Encounter；Encounter—owned by→Clinician；Encounter—may require→Procedure；Procedure—consumes→InventoryLot。为 consumes 增加数量、时间和批次，因它本质是一次使用事件。
