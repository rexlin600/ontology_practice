# Day 13｜Functions：把复杂计算包装成可测试的业务能力

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 知道何时把逻辑写成函数，并为函数设计输入、输出、测试和失败语义。
...
好函数像临床路径：输入清楚、输出可解释、没有隐式副作用、可用样本测试。

## 把它放进真实现场

风险评分可能要读连续化验、心电图结论和症状时间。不要把这段逻辑散在十个页面表达式里；封装为 calculateRisk(encounterId)，并返回分数、证据、规则版本和不确定性。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Encounter["一次就诊（Encounter）"] --> RiskFunction["RiskFunction"]
  RiskFunction["RiskFunction"] --> AssessmentEvidence["Assessment + Evidence"]
...
为 calculateRisk 设计输入、输出和两个边界测试。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

输入：encounter_id、as_of_time；输出：risk_level、score、evidence[]、data_quality、rule_version；测试：缺少 ECG 时返回 Unknown，不返回 Low；化验单位不合法时返回 InvalidData。
...
函数只返回一个布尔值。业务人员需要看到依据，工程人员需要看到失败类型。
