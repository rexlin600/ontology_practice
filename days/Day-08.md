# Day 08｜指标属于业务语义层：同一个“及时率”可能有三种算法

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 能把 KPI 写成可复算、可解释、可治理的定义。
...
指标应声明分母、分子、时间窗口、排除条件、版本、责任人。指标定义也是 Ontology 的一部分。

## 把它放进真实现场

胸痛 D2B（door-to-balloon）及时率看似简单，但“door”取分诊时间、挂号时间还是到院登记？“balloon”取设备日志还是手术记录？排除转院与无适应证患者吗？不写清，部门会各有一套数字。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Encounter["一次就诊（Encounter）"] --> Eligibility["Eligibility"]
  Eligibility["Eligibility"] --> MetricRule["MetricRule"]
  MetricRule["MetricRule"] --> KPI["KPI"]
...
为“90 分钟内再灌注率”写出分母、分子和两个排除条件。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

分母：符合 STEMI 且接受 PCI 的急诊就诊；分子：balloon_time-door_time≤90 分钟；排除：到院前已完成再灌注、临床记录明确不适合 PCI。另保留 Unknown，不能把缺时间当作不及时。
...
为了让数字好看，把未知数据并入分母或分子。Unknown 应单列，推动数据质量改进。
