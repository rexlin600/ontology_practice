# Day 27｜行业案例五：金融风控，把规则、证据与处置连起来

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 理解高监管环境下为什么规则版本、权限和审计缺一不可。
...
高风险行业强调可解释的案例管理：每个判断都要能回到证据，且不同角色看到的数据不同。

## 把它放进真实现场

一笔异常支付不是直接“拒绝”或“放行”。它会成为 Alert，对应 Transaction、Account、Device、RuleVersion 和 Investigator；调查人员需查看证据、请求补充信息、升级或结案。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Transaction["Transaction"] --> Alert["预警（Alert）"]
  Alert["预警（Alert）"] --> Investigation["Investigation"]
  Investigation["Investigation"] --> Decision["Decision"]
  Decision["Decision"] --> AuditLog["审计日志（Audit Log）"]
...
为“疑似异常转账”设计三种状态与一个人工复核点。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

New Alert→Under Review→Closed/Reported；当规则触发且金额超过阈值时，由合规分析师复核证据后才能上报。模型分数只能排优先级，不能取代法规要求的调查。
...
用一个风险分数替代案件流程。分数告诉你先看什么，流程决定你如何负责任地处理。
