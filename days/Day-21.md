# Day 21｜AIP/AI 的正确位置：先让模型读到受控事实，再让人做判断

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 理解 AI 助手应基于 Ontology 的对象与权限回答，而不是对着原始数据“自由发挥”。
...
AI 回答应携带对象链接、证据字段、时间点和不确定性；用户可以回到原始对象复核。

## 把它放进真实现场

值班医生问“还有哪些病例可能超时？”AI 可以从已授权的 Encounter、倒计时、待办和质量状态生成排序与摘要；它不应编造缺失信息，也不应自动下达医疗指令。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  UserQuestion["UserQuestion"] --> AuthorizedObjects["AuthorizedObjects"]
  AuthorizedObjects["AuthorizedObjects"] --> AIAnswerEvidence["AIAnswer + Evidence"]
  AIAnswerEvidence["AIAnswer + Evidence"] --> HumanDecision["HumanDecision"]
...
为“可能超时病例”定义 AI 回答的四个必含项。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

病例/就诊标识、剩余时限、未完成步骤、证据时间戳；若 ECG 延迟或风险未计算，明确写“数据不足，不作判断”。
