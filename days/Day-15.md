# Day 15｜用户反馈要变成证据：从吐槽到下一轮模型变更

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 学会在现场试用后归类反馈，并将其转换为模型、流程或培训改动。
...
反馈记录应有：原话、发生场景、频率、影响决策、假设原因、拟验证改动、负责人。

## 把它放进真实现场

护士说“这个红色没用”。追问后可能有三种原因：规则误报、数据延迟、颜色语义不清。它们分别需要改规则、修数据链路和改界面，不能都记为“优化体验”。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Observation["观察结果（Observation）"] --> Feedback["Feedback"]
  Feedback["Feedback"] --> Hypothesis["Hypothesis"]
  Hypothesis["Hypothesis"] --> Change["Change"]
  Change["Change"] --> Measure["Measure"]
...
可能是筛选条件缺失、排序未体现时限、对象粒度过粗。分别用默认筛选、倒计时排序、按就诊合并卡片来测试，并比较完成任务时间。
...
把单个强势用户的偏好当普遍需求。要看角色、频率和对结果的影响。
