# Day 01｜本体论不是数据字典：它让团队围绕同一个“现实”协作

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 理解 Ontology 的价值：把分散系统里的记录，变成可共同操作的业务对象。
...
对象不是表名，而是业务中可被识别、查询、讨论、授权和执行动作的实体。把它想成病历中的“患者”，而不是数据库中的一行 patient 表。

## 把它放进真实现场

急诊胸痛中心每天都在做同一件事：判断一位患者能否在 90 分钟内完成再灌注。HIS 有挂号，LIS 有肌钙蛋白，影像系统有检查，导管室有排台。若只做一张报表，医生只能看到数字；本体论把这些记录组织成“患者—就诊—检查—风险—资源—处置”的工作台。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Patient["患者（Patient）"] --> Encounter["一次就诊（Encounter）"]
  Encounter["一次就诊（Encounter）"] --> Test["检查/检验（Test）"]
  Test["检查/检验（Test）"] --> RiskAssessment["风险评估（Risk Assessment）"]
  RiskAssessment["风险评估（Risk Assessment）"] --> CareAction["协同处置（Care Action）"]
...
列出“胸痛患者能否在时限内完成分诊”所需的五个对象，并说明每个对象由哪个系统提供。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

Patient、Encounter、Test、RiskAssessment、CareAction；分别可来自 HIS、HIS、LIS/PACS、规则服务、排班/医嘱系统。关键是让临床团队确认语义，而不是由数据团队自行命名。
