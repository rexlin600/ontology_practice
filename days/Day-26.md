# Day 26｜行业案例四：供应链控制塔不等于地图大屏

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 以订单承诺为中心，连接库存、运输、客户与例外处理。
...
不要从“追踪所有车辆”开始；先找一个承诺失效时必须决策的节点。

## 把它放进真实现场

客户问“订单何时到”，答案依赖订单行、可用库存、仓库作业、承运状态和天气/口岸事件。控制塔的价值是暴露“谁要决定改仓、加急还是通知客户”。
...
flowchart LR
  ...
  Inventory["Inventory"] --> Shipment["Shipment"]
  Shipment["Shipment"] --> DelayRisk["DelayRisk"]
  DelayRisk["DelayRisk"] --> MitigationAction["MitigationAction"]
...
为“高价值订单延迟”写一个决策与三种动作。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

客户服务负责人在预计到货晚于承诺 24 小时时决定：改从备选仓发货、升级运输、或主动变更承诺。系统给出成本、库存影响和客户等级，负责人选择并留痕。
