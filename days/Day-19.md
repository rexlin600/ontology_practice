# Day 19｜协同不是消息轰炸：通知要有对象、责任和闭环

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 设计能减少漏交接的协同机制，而不是给每个人推送所有告警。
...
通知对象 = 事件 + 责任角色 + 优先级 + 期望响应 + 升级路径。

## 把它放进真实现场

当高危患者出现，系统应通知“当前负责的导管室协调员”，并把通知关联到 Encounter 和待办。接收者要么确认、要么转派、要么升级；沉默也应在超时后显性化。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  RiskEvent["RiskEvent"] --> AssignedRole["AssignedRole"]
  AssignedRole["AssignedRole"] --> AcknowledgeorReassign["Acknowledge or Reassign"]
  AcknowledgeorReassign["Acknowledge or Reassign"] --> Escalate["Escalate"]
...
为“导管室未确认”设计 10 分钟升级逻辑。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

0 分钟通知当班协调员；5 分钟未读再提醒；10 分钟未确认通知值班主管，并在 Encounter 视图标注 Escalated；确认后停止后续提醒但保留轨迹。
...
只按用户 ID 发消息。排班会变；应优先映射到角色和当前值班关系。
