# Day 18｜流程编排：状态机比“状态字段”更能避免混乱

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 能用有限状态机描述对象生命周期、允许的迁移与异常回退。
...
状态机需要定义：状态、进入条件、允许动作、退出条件、超时处理、不可逆边界。

## 把它放进真实现场

绿色通道不是一个 yes/no 字段。它可能经历 Candidate、UnderReview、Activated、InProgress、Completed、Cancelled；不同角色在不同状态才有权限执行特定 Action。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Candidate["Candidate"] --> UnderReview["UnderReview"]
  UnderReview["UnderReview"] --> Activated["Activated"]
  Activated["Activated"] --> InProgress["InProgress"]
...
为绿色通道补充 Cancelled 与 Escalated，并说明进入条件。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

Cancelled：经负责人确认不符合适应证，必须记录原因；Escalated：距时限不足且关键步骤未完成，自动通知值班负责人。二者都保留之前状态和操作者。
...
用多个布尔字段拼生命周期（is_active、is_done、is_cancelled）。组合会彼此矛盾，难以审计。
