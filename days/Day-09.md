# Day 09｜数据质量不是上线后的告警，而是模型的一部分

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 把完整性、及时性、一致性和合理性转化为可操作的质量对象。
...
为关键数据定义 SLO：例如 ECG 在采集后 3 分钟可见；缺失时显示 Unknown 与原因，而非默认正常。

## 把它放进真实现场

如果 ECG 回传延迟 30 分钟，绿色通道看板显示“低风险”并不代表患者低风险，只代表系统不知道。质量状态必须与业务状态并排呈现。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  DataFeed["DataFeed"] --> QualityCheck["QualityCheck"]
  QualityCheck["QualityCheck"] --> QualityIssue["质量问题（Quality Issue）"]
  QualityIssue["质量问题（Quality Issue）"] --> DecisionGuardrail["DecisionGuardrail"]
...
为 ECG 数据建立三条规则：延迟、缺失、格式。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

延迟：采集至到达超过 3 分钟标红；缺失：胸痛就诊 10 分钟无 ECG 生成 QualityIssue；格式：结论不在标准枚举则隔离。规则失败时禁止自动触发“低风险”。
...
只给工程团队看数据质量仪表板。受影响的业务人员必须在对象视图中看见质量状态。
