# Day 10｜血缘与可解释性：能回答“这个红灯从哪来”

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 能追踪一个结论从来源记录、转换、规则到 Action 的完整路径。
...
血缘分为数据血缘（字段从哪来）和决策血缘（规则如何得出）。二者都应可被授权查看。

## 把它放进真实现场

当医生质疑“为什么它被标为高危”，系统要能展开：哪份 ECG、哪次化验、哪个规则版本、何时计算、谁触发了 Action。没有这条链，模型只是一台黑箱。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  SourceRecord["SourceRecord"] --> TransformJob["TransformJob"]
  TransformJob["TransformJob"] --> RiskRule["风险规则（Risk Rule）"]
  RiskRule["风险规则（Risk Rule）"] --> CareAction["协同处置（Care Action）"]
  CareAction["协同处置（Care Action）"] --> AuditLog["审计日志（Audit Log）"]
...
选择一个 high_risk=true，写出最少六个可审计节点。

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

源 ECG 记录、源检验记录、映射任务版本、规则版本、计算时间、执行账号、Action 请求、Action 结果。把它们链接到 RiskAssessment 与 AuditLog。
