# Day 04｜属性要带时间：一条化验值不是患者的“永久事实”

> **学习时长：约 20–30 分钟**  
> **今天的成果：** 区分稳定属性、事件属性与计算属性，避免把时间序列压扁。
...
属性分三类：相对稳定的描述（血型）、随事件发生的事实（一次检验）、由规则计算的状态（高危标记）。事件属性应归到 TestResult 或 Observation 对象。

## 把它放进真实现场

“肌钙蛋白 = 0.08”没有意义，除非知道采样时间、单位、参考区间、检验方法和是否复核。把最近结果直接塞到 Patient 对象会遮住趋势，临床判断会被误导。

这不是“把数据画得更漂亮”。它是在**信息不完整、多人协作、必须可追责**的场景里，让下一步工作更明确。以下图是今天要掌握的最小链路：

```mermaid
flowchart LR
  Patient["患者（Patient）"] --> Observation["观察结果（Observation）"]
  Observation["观察结果（Observation）"] --> ResultAtTime["ResultAtTime"]
...
为“高危胸痛”设计字段：哪些是原始事实，哪些是计算结果？

建议先在纸上完成。写不出来时，不要急着查工具：通常说明你还没有把“现实中的一个决定”说清楚。

## 参考答案与检查清单

原始事实：年龄、症状起始时间、ECG 结论、连续肌钙蛋白结果；计算结果：risk_score、risk_level、rule_version、calculated_at。计算结果必须保留规则版本。
...
只保留“最新值”。当决策与趋势有关时，最新值会制造错误的确定感。
